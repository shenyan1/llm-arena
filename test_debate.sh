...
# 合法参数 → 退出码 0
PATH="/tmp/debate_mock:$PATH" bash "$SCRIPT_DIR/debate.sh" "测试辩题" 3 >/dev/null 2>&1; EXIT=$?; assert_exit "合法参数时成功退出" 0 $EXIT
...
# 设置 mock
mkdir -p /tmp/debate_mock
cat > /tmp/debate_mock/claude << 'MOCK'
#!/bin/bash
echo "Claude 模拟回应，包含论点。"
echo "WINNER: 正方"
MOCK
cat > /tmp/debate_mock/gemini << 'MOCK'
#!/bin/bash
echo "Gemini 模拟回应，包含反驳。"
echo "WINNER: 正方"
MOCK
chmod +x /tmp/debate_mock/claude /tmp/debate_mock/gemini
...
echo "$output" | grep -q "第 1 轮" && echo "✓ 包含第1轮标题" || { echo "✗ 缺少第1轮标题"; FAIL=$((FAIL + 1)); }
echo "$output" | grep -q "第 2 轮" && echo "✓ 包含第2轮标题" || { echo "✗ 缺少第2轮标题"; FAIL=$((FAIL + 1)); }
...
if [ -n "$md_file" ]; then
    ...
    grep -q "## 第 1 轮" "$md_file" && echo "✓ 文件包含第1轮" || { echo "✗ 文件缺少第1轮"; FAIL=$((FAIL + 1)); }
